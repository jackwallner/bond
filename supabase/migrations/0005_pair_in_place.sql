-- Pair-in-place: a solo user must be able to pair with a partner WITHOUT
-- losing their existing reminders/milestones.
--
-- The old consume_invite_code rejected anyone "already in a couple" and the
-- only escape (leave_couple) does `delete from couples`, which cascades to
-- reminders/milestones. Since every user now starts in a solo couple by
-- default, that path would silently destroy their data on first pairing.
--
-- This migration:
--   1. adds profiles.love_language (synced from on-device prefs at pair time)
--   2. replaces consume_invite_code with a merge: it reuses one couple row,
--      migrates the other user's rows onto it, and deletes the now-empty
--      solo couple. Real (non-solo) couples on either side still reject.
