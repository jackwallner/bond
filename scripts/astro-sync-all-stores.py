...
def build_keywords_for_locales(locale_dirs: list[Path], store_code: str) -> list[str]:
    ...
    if store_code == "us" and EN_US_CURATED.exists():
        ...
        items.extend(json.loads(EN_US_CURATED.read_text()).get("keywords", [])[:20])
...
def main() -> None:
    ...
    parser.add_argument("--resume", action="store_true", help="Skip stores with existing per-store JSON")
    ...
    all_store_count = len(json.loads(STORES_JSON.read_text())["stores"])
    summary_path = OUT_DIR / "_summary.json"
    if args.store and summary_path.exists():
        summary = json.loads(summary_path.read_text())
        summary.setdefault("stores", {})
    else:
        summary = {
            "syncedAt": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
            "appId": app_id,
            "storeCount": all_store_count,
            "stores": {},
        }
    ...
    for store, info in sorted(plan.items()):
        ...
        if args.resume and not args.dry_run:
            try:
                existing_kws = call(MCP_URL, "get_app_keywords", {"appId": app_id, "store": store})
                n_existing = len(existing_kws) if isinstance(existing_kws, list) else 0
                need = max(12, int(len(keywords) * 0.65))
                if n_existing >= need:
                    print(f"    resume: skip ({n_existing} keywords in Astro)")
    ...
    summary["syncedAt"] = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    summary["appId"] = app_id
    summary["storeCount"] = all_store_count
